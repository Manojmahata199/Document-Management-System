...
class Welcome extends CI_Controller {
	...
	public function delete_row($user_id) {
	    ...
	    if($data==true){
	    	...
	    	$this->session->set_flashdata('success_msg','The User Deleted Succesfully');
		...
		if ($this->form_validation->run()==false) {
			...
			if($_POST['submit']){
		    	...
		    	$doc_file_path=$_FILES['doc_file']['tmp_name'];
		    	   ...
		    	   if($_FILES['doc_file'])
					{
							if (!file_exists('document_uploads/'.$doc_tittle.'/'))
							{
                              ...
                              $target_dir = 'document_uploads/'.$doc_tittle.'/';
							  $target_file = $target_dir.$_FILES["doc_file"]["name"];


							  $extension = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
                              $new_path='document_uploads/'.$doc_tittle.'/'.$doc_file_name.'';
							...
							if($uploadOk ==1)
							{
								$d=mkdir('document_uploads/'.$doc_tittle.'/', 0777, true);								
    ...
    public function download($id){
	        ...
	        force_download('document_uploads/'.$folder.'/'.$file_name.'',null);
